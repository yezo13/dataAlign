...
{
	mask = Mat(cur_left_img.rows, cur_left_img.cols, CV_8UC1, Scalar(255));  //here is gray is CV_8UC1 and color is CV_8UC3 I CHANGE IT 
	 	...
	 	if(ii != ids.size())
	 	{
		 	...
		 	*/
		 	...
		 	for(int i = 0; i < WINDOW_SIZE-1; ++i)
		 	{
				...
				bool ifRepeat = false;
				for(it = allRp.begin();it != allRp.end();it++){
				    if((*it)(0,0) == Rp[0](0,0) && (*it)(0,1) == Rp[0](0,1)){
				       ifRepeat = true;
				       break;
				    }
				}
				if(!ifRepeat)
				  allRp.push_back(Rp[0]); // push all the frame will have been replace until the last 10 frames
		 		Rp[i] = Rp[i+1];
		 		tp[i] = tp[i+1];
		 	}
		 	++frameID0;
